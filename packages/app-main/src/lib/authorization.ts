 ...
 * ```
 *
 * For LLM routes with rate limiting:
 * ```typescript
 * const authResult = await requireAuth({ rateLimit: "expensive_llm" });
 * ```
 *
 * For unauthenticated routes (login/signup) with IP-based rate limiting:
 * ```typescript
 * const authResult = await requireAuth({
 *   permissions: { allowUnauthenticated: true },
 *   rateLimit: "standard_llm"
 * }, request);
 * ```
 */
