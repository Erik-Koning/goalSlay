...
export const SuperAppAdminEmails = process.env.NEXT_PUBLIC_SUPER_APP_ADMIN_EMAILS?.split(",") ?? ["erik", "jake", "jacob", "cody"];

export function emailIsSuperAppAdmin(
  email?: string,
  domain: string | (string | undefined)[] | undefined = [process.env.NEXT_PUBLIC_APP_DOMAIN_NAME, process.env.NEXT_PUBLIC_MARKETING_DOMAIN_NAME]
): boolean {
  debugger;
  if (!email || !domain) return false;
  ...
  if (Array.isArray(domain)) {
    // Only return false if email does not end with ANY of the domains
    if (!domain.some((d) => d && email.endsWith(d.toLowerCase()))) return false;
  } else {
    if (!email.endsWith(domain.toLowerCase())) return false;
  }
