...
export async function sendNotifyNewUserTeamEmail(
  ...
  const recipient = `hello@${process.env.NEXT_PUBLIC_DOMAIN}`; //TODO: change to hello@${process.env.NEXT_PUBLIC_DOMAIN}
  const sender = senderProvided ?? `verify@${process.env.NEXT_PUBLIC_DOMAIN}`;
