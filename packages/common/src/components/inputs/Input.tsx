...
import { EyeOff, Eye, EditIcon } from "lucide-react";
...
const inputVariants = cva(
  "pl-[10px] peer transition-all w-full rounded-md focus:ring-1 focus:ring-skyBlue border-[1.5px] border-border hover:border-border-hovered",
  {
    variants: {
      variant: {
        default:
          "bg-background text-primary border-[1.5px] border-border font-normal outline-none disabled:hover:border-border disabled:cursor-not-allowed disabled:text-secondary-dark disabled:opacity-75 disabled:bg-secondary-light",
        ...
        shortText:
          "bg-background text-primary hover:border-neutral-500 border border-primary font-normal outline-none disabled:cursor-not-allowed disabled:opacity-70 w-full max-w-[400px]",
...
const checkboxAndRadioVariants = cva(
  "form-checkbox cursor-pointer disabled:bg-secondary-dark transition-all bg-background border-[1px] rounded-[2px] border-[1.5px] border-border hover:border-secondary-dark disabled:cursor-not-allowed disabled:opacity-70",
  {
    variants: {
      variant: {
        default:
          "bg-background text-primary border-[1.5px] border-border hover:border-secondary-dark font-normal outline-none disabled:cursor-not-allowed disabled:opacity-70",
...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    const handleInputChange = (e: any) => {
      ...
      // DEBUG: Log initial event
      if (type === "number") {
        console.log("[INPUT handleInputChange] START", { id, type, rawValue: e?.target?.value, rawType: typeof e?.target?.value });
      }
      ...
      if (type === "checkbox") {
        ...
        if (type === "number") {
          console.log("[INPUT] After String():", { localInputValue, type: typeof localInputValue });
        }
        ...
        // Handle number type: filter non-numeric characters
        if (type === "number") {
          // Allow only digits, decimal point, and minus sign at the start
          // First, preserve minus sign if at start
          const hasLeadingMinus = localInputValue.startsWith("-");
          // Remove all non-numeric characters except decimal point
          localInputValue = localInputValue.replace(/[^0-9.]/g, "");
          // Only allow one decimal point
          const parts = localInputValue.split(".");
          if (parts.length > 2) {
            localInputValue = parts[0] + "." + parts.slice(1).join("");
          }
          // Re-add leading minus if it was there
          if (hasLeadingMinus && localInputValue.length > 0) {
            localInputValue = "-" + localInputValue;
          }
          console.log("[INPUT] After number filtering:", { localInputValue, type: typeof localInputValue });
        }
        ...
        // For number type, convert to number for the event value
        const eventValue = type === "number" && localInputValue !== "" && localInputValue !== "-" ? Number(localInputValue) : localInputValue;
        
        // DEBUG: Log event value before generating event
        if (type === "number") {
          console.log("[INPUT] eventValue created:", { eventValue, type: typeof eventValue, localInputValue });
        }
        
        e = generateEvent({ value: eventValue, name: registerName ?? id ?? name });
        
        // DEBUG: Log generated event
        if (type === "number") {
          console.log("[INPUT] Generated event:", { value: e.target.value, type: typeof e.target.value, name: e.target.name });
        }
      ...
      // DEBUG: Log before calling onChange handlers
      if (type === "number") {
        console.log("[INPUT] Before calling handlers:", {
          hasOnChange: !!onChange,
          hasRegisterOnChange: !!registerOnChange,
      ...
      // DEBUG: Log after calling handlers
      if (type === "number") {
        console.log("[INPUT handleInputChange] END");
      }
    };
    ...
    const mainJSX = (
      ...
      >
        ...
        <div
          className={cn(
            "flex gap-x-1 pb-0 text-primary",
            {
        ...
        >
          <LabelAbove label={labelAbove} labelClassName={labelClassName} className={cn({ "-mb-[3px]": options && options.length > 0 }, labelClassName)} />
        ...
        <div className={cn("relative", { "flex items-center": type === "checkbox" }, inputContainerClassName)}>
          <input
            ...
            onKeyDown={(e) => {
              ...
              // For number type, prevent non-numeric characters
              if (type === "number") {
                const allowedKeys = ["Backspace", "Delete", "Tab", "Escape", "Enter", "ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown", "Home", "End"];
                const isAllowedKey = allowedKeys.includes(e.key);
                const isNumber = /^[0-9]$/.test(e.key);
                const isDecimal = e.key === "." && !inputValue.includes(".");
                const isMinus = e.key === "-" && inputValue.length === 0;
                const isSelectAll = (e.metaKey || e.ctrlKey) && e.key === "a";
                const isCopyPaste = (e.metaKey || e.ctrlKey) && ["c", "v", "x"].includes(e.key);

                if (!isAllowedKey && !isNumber && !isDecimal && !isMinus && !isSelectAll && !isCopyPaste) {
                  e.preventDefault();
                }
              }
            ...
            ref={(node) => {
              // Include the forwarded ref from React.forwardRef to ensure react-hook-form's ref works
              mergeRefs(node, ref, inputRef, registerRef, innerRef, localInputRef);
            }}
              ...
              {type === "password" && allowVisibility && (
                ...
                >
                  {showPassword ? (
                    ...
                    <EyeOff size={17} className="transform -scale-x-100" />
                  )}
                </Button>
              )}
            </div>
