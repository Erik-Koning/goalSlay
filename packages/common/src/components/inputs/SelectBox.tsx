...
import { X as CloseOutlinedIcon, Edit } from "lucide-react";
...
const inputVariants = cva("", {
  variants: {
    variant: {
      default: "",
      purple: "bg-purple hover:bg-darkPurple text-primary-light w-full justify-center",
  ...
  (
    {
      ...
      inputBoxClassName,
      focusBorderColor,
      outlineColor = "var(--skyBlue)",
      selectHoverColor = "var(--faintPurple)",
    ...
    const OptionTypeKeys: Array<keyof OptionType> = ["value", "label", "color", "isFixed", "isDisabled", "email"];
    ...
    */
    ...
    const [hoveringOverOption, setHoveringOverOption] = useState<string | undefined>(undefined);
    ...
    useEffect(() => {
      ...
      if (!found && JSXBelow) setJSXBelow(null);
    ...
    const removeValueFromState = (value: string) => (e: React.MouseEvent) => {
      e.stopPropagation();
      e.preventDefault();
              ...
              >
                <CreatableSelect
                  ...
                  formatOptionLabel={(option: Record<string, any>, formatOptionLabelMeta: FormatOptionLabelMeta<any>) => (
                    <div
                      onMouseEnter={() => {
                        setHoveringOverOption(option.value);
                      }}
                      onMouseLeave={() => {
                        setHoveringOverOption(undefined);
                      }}
                      className={cn("flex flex-row items-center pl-[2px] text-center", allowEditButton && isStatic && "hover:cursor-pointer")}
                    >
                      {(() => {
                        return null;
                      })()}
                      ...
                      {isMulti && removeMultiInLabel && Array.isArray(valueState) && getObjectWithKeyValue(valueState, "value", option.value) && (
                        ...
                        <Button
                          ...
                          disabled={hoveringOverOption !== option.value}
                          onClick={(e: any) => {
                            if (hoveringOverOption !== option.value) return;
                            removeValueFromState(option.value)(e);
                          }}
                          stopMouseEventPropagation={true}
                          preventInitialClick={250}
                          //ignoreFirstClick={true}
                        >
                          <CloseOutlinedIcon size={22} className="rounded-full p-0.5 text-secondary-dark hover:bg-border" />
                        </Button>
                      )}
                    </div>
                  )}
                  ...
                  styles={{
                    ...
                    option: (provided, state) => ({
                      ...
                      minWidth: "100%", // Ensure hover background spans full scroll width
                      width: "max-content", // Allow option to expand to content width
