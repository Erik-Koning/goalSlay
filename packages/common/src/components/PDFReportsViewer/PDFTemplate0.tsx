...
import { falseOrEmpty, getValueByKeyRecursive, objHasKey, removeKeysAndFlatten, retainKeys, retainKeysWithValues } from "../../utils/objectManipulation";
import { Field, TemplateTabData } from "../../types/formGenerator";
import { getRegardingValue, RegardingValue } from "../../utils/form";
import { DateFormat, getDateFromDateStringFormat } from "@common/utils/dateManipulation";
...
const PDFTemplate0: React.FC<SinglePDFPreviewProps> = ({
  ...
  //Get an appointment date to date this report was created
  const x = entryFields;
  debugger;
  let appointmentDateStr: string | undefined = getValueByKeyRecursive(entryFields, "appointmentDate");
  debugger;
  const appointmentDate = appointmentDateStr ? getDateFromDateStringFormat(appointmentDateStr, officeFields.dateFormat as DateFormat | null) : undefined;
    ...
    >
      <Page size="LETTER" style={styles.page} wrap debug={debug}>
        ...
        <View key={title + "contentContainer"} style={[styles.contentContainer, { flexGrow: appendixFieldsPageBreak ? 0 : 1 }]} wrap debug={debug}>
          <Recipient
            lastEdited={appointmentDate ?? lastEdited}
            patientName={`${entryFields.patient.firstName} ${entryFields.patient.lastName}`}
            patientDOB={entryFields.patient.dateOfBirth}
