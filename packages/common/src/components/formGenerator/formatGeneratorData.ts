...
// A referenced field is one that has a _ between two words, like {patient_firstName}
export const referencedFieldsRegex = new RegExp(/\{[A-Za-z0-9]+_[A-Za-z0-9]+[^}]*\}/g);

// Global placeholder keys for form field replacements
export const GENERIC_PLACEHOLDER_KEYS = {
  TODAY: "{TODAY}",
  NOW: "{NOW}",
  USER: "{USER}",
} as const;

export type PlaceholderKey = (typeof GENERIC_PLACEHOLDER_KEYS)[keyof typeof GENERIC_PLACEHOLDER_KEYS];
export const lazyReferencedFieldsRegex = new RegExp(/{([^{}]+)}/g);
export const singularReferencedFieldsRegex = new RegExp(/{([^{}_]+)}/g);
...
const formatFormGeneratorFields = (
  ...
  const replacements: Record<string, string> = {
    [GENERIC_PLACEHOLDER_KEYS.TODAY]: getStringDateFormatFromDate(new Date(), config.dateFormat),
    [GENERIC_PLACEHOLDER_KEYS.NOW]: new Date().toLocaleTimeString(),
    [GENERIC_PLACEHOLDER_KEYS.USER]: "{relUser_prefix} {relUser_firstName} {relUser_lastName}",
  };
