...
import { CloudOff as CloudOffOutlined, Cloud as FilterDramaOutlined, Cloud as CloudDoneOutlined, RefreshCcw as AutorenewOutlined, Check } from "lucide-react";
...
export const CloudStateIndicator = () => {
  ...
  const renderIcon = () => {
    switch (globCloudState.cloudState) {
      case -2:
        return <FilterDramaOutlined className={`text-red-500 ${isShaking ? "animate-shake" : ""}`} size="1em" />;
      case -1:
        return <CloudOffOutlined className="text-red-500" size="1em" />;
      case 0:
        return <FilterDramaOutlined size={25} className="text-[20px] text-secondary-dark" />;
      case 1:
        return (
          <div className="relative text-secondary-dark">
            <FilterDramaOutlined size={25} className="" />
            <div className="absolute right-[6px] top-[22px] h-[14px] w-[14px] items-center justify-center rounded-full bg-background text-[14px]">
              <AutorenewOutlined className="absolute animate-spin rounded-full" size="1em" />
            </div>
          </div>
        );
      case 2:
        return (
          <div className="relative text-green">
            <FilterDramaOutlined size={25} className="mb-[2px]" />
            <div className="absolute right-[-2px] bottom-[-2px] bg-background rounded-full">
              <Check size={12} />
            </div>
          </div>
        );
