...
const selectTriggerVariants = cva(
  "flex h-[38px] transition-all w-full items-center justify-between rounded-md px-3 py-2 text-sm ring-offset-background placeholder:text-primary focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 hover:cursor-pointer",
  {
    variants: {
      variant: {
        default: "border border-input bg-background hover:border-border-hovered",
...
const SelectTrigger = React.forwardRef<React.ComponentRef<typeof SelectPrimitive.Trigger>, SelectTriggerProps>(
  ({ className, children, chevronClassName, chevronIcon, variant, ...props }, ref) => (
    <SelectPrimitive.Trigger ref={ref} className={cn(selectTriggerVariants({ variant, className }))} {...props}>
      {children}
      <SelectPrimitive.Icon asChild>
        {chevronIcon ? chevronIcon : <ChevronDown className={cn("h-4 w-4 opacity-50", chevronClassName)} />}
...
export {
  ...
  SelectScrollUpButton,
  SelectScrollDownButton,
  selectTriggerVariants, // Exporting variants in case needed elsewhere
};
