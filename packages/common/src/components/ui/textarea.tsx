...
import { X, RotateCw, Redo2, Building2, History, Undo2 } from "lucide-react";
...
const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(
  (
    ...
    useKeysPressed(
      ...
      isHovered || isFocused || AIEditMode,
      undefined,
      undefined,
      undefined,
    ...
    const handleInputChange = (e: any) => {
      ...
      if (!isLinesSameLength && enableBulletPoints) {
        let lineIndex = -1;
    ...
    const generateFieldValueAI = async (userPrompt: string = "", systemPrompt?: string) => {
      ...
      if (aiActionsAdded.length === 1 && aiActionsAdded[0].text === fillByPromptString) {
        if (!prompt) {
          ...
          userPrompt += aiActionsAdded[0].prompt;
        ...
        const userEditedFieldsString = getUserEditedFields ? getUserEditedFields() : "";
        if (userEditedFieldsString) {
          userPrompt += userEditedFieldsString;
          userPrompt +=
            "\nWhen referencing placeholder values only use placeholder variables included above, do not make up new placeholder variables, and always stick to the placeholder syntax of {variable_name} ex: {patient_firstName}, NOT: {firstName} unless a field is listed in this way";
        }
      } else {
        userPrompt =
        ...
        for (let i = 0; i < aiActionsAdded.length; i++) {
          if (i > 0) userPrompt += " also ";
          userPrompt += aiActionsAdded[i].prompt ?? aiActionsAdded[i].text;
        }
        userPrompt += ".\n\n";
        userPrompt += anonymizeStringWithReferencedFields(textValueState, anonymizeFields);
      }

      if (!systemPrompt) {
        systemPrompt = "You are a professional dental assistant, filling out a consult form\n";
      }
    ...
    return (
      <div ref={divRef} className={cn("relative", className)}>
              ...
              <div className="flex items-center gap-x-2">
                ...
                {aiEdits && (
                  <HoverCardClickable
                    ...
                    side={"top"}
                    //forceOpen={true}
                    triggerJSX={<RotateCw style={{ fontSize: 14 }} className={labelIconsClassNames} onClick={toggleAIEditMode} />}
                  ...
                  >
                    <div className="flex items-center gap-x-1 px-2 py-1 text-sm text-primary-dark">
                      <p>AI Actions</p>
                      <span className="h-fit rounded-md bg-faintPurple p-0.5 px-1 text-xs">{isMac ? "⌘" : "Ctrl"}</span>
                      <span className="h-fit rounded-md bg-faintPurple p-0.5 px-1 text-xs">{isMac ? "⌘ + C" : "Ctrl + C"}</span>
                    </div>
                  </HoverCardClickable>
                )}
              </div>
                ...
                >
                  {preAIValue && preAIValue !== textValueState && (
                    <Button id="undoButton" className={cn("", textValueState && "bg-background/75")} variant="ghost" onClick={undoAIAction}>
                      {undoIsRedo ? (
                        <Redo2 className={cn(" h-4 w-4 transition-all duration-200 dark:text-white")} />
                      ) : (
                        <Undo2 className={cn(" h-4 w-4 transition-all duration-200 dark:text-white")} />
                      )}
                    </Button>
                  )}
        ...
        <textarea
          ...
          className={cn(
            "peer h-full w-full appearance-none border border-primary bg-background p-2 outline outline-1 outline-transparent transition-all hover:border-neutral-400 focus:outline-skyBlue disabled:cursor-not-allowed disabled:bg-secondary-light disabled:text-secondary-dark disabled:opacity-75 disabled:hover:border-border",
            {
