...
export function DataTable<TData, TValue>({
  ...
  const [navbarHeight, setNavbarHeight] = React.useState(64); // default fallback
  const [headerHeight, setHeaderHeight] = React.useState(48); // default fallback

  // Compute CSS variable values on client only
  React.useEffect(() => {
    if (typeof window === "undefined") return;
    const computedNavbar = parseInt(window.getComputedStyle(document.documentElement).getPropertyValue("--navbar-height"), 10);
    const computedHeader = parseInt(window.getComputedStyle(document.documentElement).getPropertyValue("--header-height"), 10);
    if (!isNaN(computedNavbar)) setNavbarHeight(computedNavbar);
    if (!isNaN(computedHeader)) setHeaderHeight(computedHeader);
  }, []);
              ...
              <TableBody>
                {data && data.length > 0 && table.getRowModel().rows?.length ? (
                  ...
                  table.getRowModel().rows.map((row) => {
                    ...
                    // Safely get the row value for 'id', fallback to '_id' if not present
                    let idRowValue: unknown;
                    if (row.getAllCells().some(cell => cell.column.id === "id")) {
                      idRowValue = row.getValue("id");
                    } else if (row.getAllCells().some(cell => cell.column.id === "_id")) {
                      idRowValue = row.getValue("_id");
                    } else {
                      idRowValue = undefined;
                    }
        ...
        <div className={cn("flex items-center justify-between px-0.5 pt-2", { "pb-4": fitPageSizeMode === false })}>
          ...
          <div className="flex items-center text-secondary-dark lg:space-x-10">
            <div className="flex items-center space-x-2">
              ...
              >
                <SelectTrigger className="h-8 w-full border-[1.5px]">
                  {fitPageSizeMode ? "Fit" : table.getState().pagination.pageSize}
                </SelectTrigger>
